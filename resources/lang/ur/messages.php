<?php

use App\Models\Country;

return [
'breaking_news' => 'اہم ترین خبریں',
    'category'=> 'زمرہ',
    'Related to this News'=> ' اس خبر سے متعلق',
    'latest_blogs' => 'تازہ ترین بلاگز',
    'live_news'=> 'لائیو خبریں',
    'prev'=> 'پچھلا',
    'next'=> 'اگلا',
    'recent_news'=> 'حالیہ خبریں',
    'category_of_this_news'=> 'اس خبر کا زمرہ',
    'share'=> 'شیئر کریں',
    'no_related_news_found_for_this_topic'=> 'اس موضوع سے متعلق کوئی خبر نہیں ملی',
    'prev_article'=> 'پچھلا مضمون',
    'next_article'=> 'اگلا مضمون',
   'latest_news' => 'حالیہ خبریں',
    'related_news'=> 'متعلقہ خبریں',
    'recent_breaking_news'=> 'حالیہ بریکنگ نیوز',
    'latest'=> 'تازہ ترین',
    'all'=> 'تمام',
    'read_more'=> 'مزید پڑھیں',
    'load_more_posts'=> 'مزید پوسٹس لوڈ کریں۔',
    'sample_popular_post_title'=> 'نمونہ مقبول پوسٹ کا عنوان',
    'Video_not_available_Please_check_the_URL'=> 'ویڈیو دستیاب نہیں ہے۔ براہ کرم URL چیک کریں۔',
    'all_blogs'=> 'تمام بلاگز',
    'privacy_policy'=> 'پالیسی',
    'terms_of_use'=> 'استعمال کی شرائط',
    'about_us'=> 'ہمارے بارے میں',
    'contact_us'=> 'ہم سے رابطہ کریں۔',
    'other_pages'=> 'دوسرے صفحات',
    'about_page'=> 'ہمارے بارے میں - ڈیلی نیوز',
    'contact_text'=> 'کوئی سوال ہے، کہانی کا مشورہ ہے، یا تاثرات بانٹنا چاہتے ہیں؟
نیچے دیئے گئے فارم کو پُر کریں، اور ہماری ٹیم جلد از جلد آپ سے رابطہ کرے گی۔',
'get_in_touch'=> 'رابطہ کریں',
'address'=> 'پتہ: غوری ٹاؤن اسلام آباد',
'phone'=> 'فون: 923065533444+',
'email'=> 'ای میل',
'name'=> 'نام',
'subject'=> 'موضوع',
'mobile'=> 'موبائل',
'message'=> 'پیغام',
'submit'=> 'جمع کریں',

    // 'to'=> 'کو',
    // 'total_News_by_this_author'=> 'اس مصنف کی طرف سے کل خبریں',

];
return [
    'categories' => [
        'all'=> 'تمام',
        'business' => 'کاروبار',
        'sports' => 'کھیل',
        'entertainment' => 'تفریح',
        'technology' => 'ٹیکنالوجی',
        'health' => 'صحت',
        'politics'=> 'سیاست',
        'tech'=> 'ٹیکنالوجی',
        'food'=> ' کھانا',
        ''=> '',
    ],
];