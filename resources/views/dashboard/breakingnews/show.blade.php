...
@section('main_section')
                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <div>
                                <h2 class="h3 mb-2 text-white"><i class="fas fa-newspaper me-3" aria-hidden="true"></i>
                                    Breaking News Management Dashboard</h2>
                                <p class="mb-0 text-white-50 fs-5">Manage all Breaking News and their information</p>
                            </div>
                            <div class="d-flex gap-3 mt-3 mt-md-0">

                                {{-- Accessible Add New News Button --}}
                                <a href="{{ route('breakingnews.add') }}"
                                    class="btn btn-light btn-lg rounded-pill px-4 py-2 shadow-sm" id="addNewsBtn"
                                    aria-label="Add New News">
                                    <i class="fas fa-plus-circle me-2" aria-hidden="true"></i> Add Breaking News
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="global-stats">
                            @php
                                ...
                                $totalBreakingNews = $breakingNews->count(); // Use count() for collections
                                $activeBreakingNews = $breakingNews->where('breakingnews_status', 'active')->count();
                                $inactiveBreakingNews = $breakingNews
                                    ->where('breakingnews_status', 'inactive')
                                    ->count();
                                $recentlyAddedBreakingNews = $breakingNews
                            ...
                            <div class="stat-item">
                                <div class="stat-value">{{ $totalBreakingNews }}</div>
                                <div class="stat-label">Total News</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value">{{ $activeBreakingNews }}</div>
                                <div class="stat-label">Active News</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value">{{ $inactiveBreakingNews }}</div>
                                <div class="stat-label">Inactive News</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value">{{ $recentlyAddedBreakingNews }}</div>
                                <div class="stat-label">Recently Added</div>
                            </div>
                        </div>

                        {{-- Check if the $breakingNews collection is empty --}}
                        @if (isset($breakingNews) && $breakingNews->isEmpty())
                            <div class="empty-state">
                                <div class="mb-5">
                                    <i class="fas fa-newspaper empty-state-icon"></i>
                                </div>
                                <h4 class="h3 text-muted mb-4">No Breaking News Found</h4>
                                <p class="text-muted fs-5 mb-5">Get started by adding your first Breaking News article to
                                    the system
                                </p>
                                <a href="{{ route('breakingnews.add') }}"
                                    class="btn btn-primary btn-lg px-5 rounded-pill shadow-sm"
                                    aria-label="Create First News">
                                    <i class="fas fa-plus-circle me-2"></i> Create First Breaking News
                                </a>
                            </div>
                        @else
                                            ...
                                            <tr class="border-top col-12">
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <div>
                                                            <h6 class="mb-1 fw-bold fs-5">{{ $item->title }}</h6>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <div>
                                                            ...
                                                            <img src="{{ asset('breakingnews_images/images/' . $item->image) }}"
                                                ...
                                                <td>
                                                    @if ($item->breakingnews_status == 'active')
                                                        <span
                                                            ...
                                                            {{ $item->breakingnews_status }}
                                                        </span>
                                                    @elseif ($item->breakingnews_status == 'inactive')
                                                        <span
                                                ...
                                                <td class="pe-4 text-end">
                                                    <div class="d-flex justify-content-between gap-1 action-buttons">
                                                        ...
                                                        <a href="{{ route('breakingnews.edit', $item->id) }}"
                                                            ...
                                                            aria-label="Edit Breaking News {{ $item->title }}">
                                                            <i class="fas fa-edit me-1" aria-hidden="true"></i> Edit
                                                        </a>
                                                        {{-- Delete Form (kept as is, it's correct for a DELETE request) --}}
                                                        <form action="{{ route('breakingnews.delete', $item->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit"
    ...
    <style>
        ...
        /* Ensure DataTables container doesn't hide elements if you manually add them */
        .dt-container .justify-content-between {
            /* This rule might hide default DataTables controls. Adjust if needed. */
            /* If you want to customize DataTables layout, do it via its options, not by hiding default elements like this unless you're replacing them. */
