                        ...
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <div>
                                <h2 class="h3 mb-2 text-white"><i class="fas fa-user-secret me-3" aria-hidden="true"></i>
                                    Policy Management Dashboard</h2>
                                <p class="mb-0 text-white-50 fs-5">Manage all Policy information</p>
                            </div>
                            <div class="d-flex gap-3 mt-3 mt-md-0">
                                <a href="{{ route('privacy.add') }}"
                                    class="btn btn-light btn-lg rounded-pill px-4 py-2 shadow-sm" id="addVideoBtn"
                                    aria-label="Add New Video">
                                    <i class="fas fa-plus-circle me-2" aria-hidden="true"></i> Add New Policy
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        @if ($privacies->isEmpty())
                            <div class="empty-state">
                                <div class="mb-5">
                                    <i class="fas fa-user-secret empty-state-icon"></i>
                                </div>
                                <h4 class="h3 text-muted mb-4">No Policies Found</h4>
                                <p class="text-muted fs-5 mb-5">Get started by adding your first Policy to the system</p>
                                <a href="{{ route('privacy.add') }}"
                                    class="btn btn-primary btn-lg px-5 rounded-pill shadow-sm"
                                    aria-label="Create First Video">
                                    <i class="fas fa-plus-circle me-2"></i> Create First Policy
                                </a>
                            </div>
                        @else
                                ...
                                <table id="videosTable" class="table align-middle mb-0 display">
                                    ...
                                    <tbody>
                                        @foreach ($privacies as $privacy)
                                            <tr class="border-top">
                                                <td class="ps-4 fw-bold text-muted fs-5">{{ $privacy->id }}</td>
                                                ...
                                                <td class="pe-4 text-end">
                                                    <div class="d-flex justify-content-end gap-2 action-buttons">
                                                        <a href="{{ route('privacy.edit', $privacy->id) }}"
                                                            ...
                                                            aria-label="Edit Video {{ $privacy->title }}">
                                                            <i class="fas fa-edit me-1" aria-hidden="true"></i> Edit
                                                        </a>
                                                        {{-- Corrected View Button --}}
                                                        <a href="{{ route('privacy.view', $privacy->id) }}"
...
>
                                                        ...
                                                        <form action="{{ route('privacy.delete', $privacy->id) }}"
                                ...
                                <div class="pagination-container">
                                    <nav aria-label="Videos pagination">
                                        <ul class="pagination">
                                            ...
                                            @else
                                                <li class="page-item">
                                                    <a class="page-link" href="{{ $privacies->previousPageUrl() }}"
                                                        rel="prev">
                                                        <i class="fas fa-chevron-left"></i>
                                                        <span class="d-none d-sm-inline ms-2">Prev</span>
                                                    </a>
                                            ...
                                            {{-- Pagination Elements --}}
                                            @foreach ($privacies->getUrlRange(1, $privacies->lastPage()) as $page => $url)
                                                @if ($page == $privacies->currentPage())
                                            ...
                                            {{-- Next Page Link --}}
                                            @if ($privacies->hasMorePages())
                                                <li class="page-item">
                                                    <a class="page-link" href="{{ $privacies->nextPageUrl() }}"
                                                        rel="next">
