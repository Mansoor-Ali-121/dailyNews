                ...
                <div class="card border-0 shadow-lg overflow-hidden">
                    <div class="card-header position-relative" style="background: var(--primary-gradient);">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <div>
                                <h2 class="h3 mb-2 text-white"><i class="fas fa-layer-group me-3" aria-hidden="true"></i>
                                    Category Management Dashboard</h2>
                                <p class="mb-0 text-white-50 fs-5">Manage all categories and their information</p>
                            </div>
                            <div class="d-flex gap-3 mt-3 mt-md-0">
                                {{-- Accessible Add New Category Button --}}
                                <a href="{{ route('category.add') }}"
                                    class="btn btn-light btn-lg rounded-pill px-4 py-2 shadow-sm" id="addCategoryBtn"
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="global-stats">
                            @php
                                ...
                                $activeCategories = $categories->where('category_status', 'active')->count();
                                $inactiveCategories = $categories->where('category_status', 'inactive')->count();
                                $recentlyAddedCategories = $categories
                                    ->filter(function ($category) {
                                        return now()->diffInDays($category->created_at) <= 30;
                                    })
                                    ->count();
                            @endphp
                            ...
                            <div class="stat-item">
                                <div class="stat-value">{{ $recentlyAddedCategories }}</div>
                        ...
                        @if ($categories->isEmpty())
                            <div class="empty-state">
                                ...
                                <a href="{{ route('category.add') }}" class="btn btn-primary btn-lg px-5 rounded-pill shadow-sm"
                                    aria-label="Create First Category">
                                    <i class="fas fa-plus-circle me-2"></i> Create First Category
                                </a>
                            </div>
                        @else
                            <div class="table-responsive">
                                {{-- Add id="categoriesTable" for DataTables initialization --}}
                                <table id="categoriesTable" class="table align-middle mb-0 display">
                                    <thead class="bg-light">
                                        <tr class="text-center">
                                            <th class="ps-4 py-3 text-uppercase fw-bold text-muted fs-5 border-0">ID</th>
                                            <th class="py-3 text-uppercase fw-bold text-muted fs-5 border-0">Category Name
                                            </th>
                                            <th class="py-3 text-uppercase fw-bold text-muted fs-5 border-0">Category Slug
                                            </th>
                                    ...
                                    <tbody>
                                        @foreach ($categories as $category)
                                            <tr class="border-top">
                                                <td class="ps-4 fw-bold text-muted fs-5">{{ $category->id }}</td>
                                                <td>
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        <small class="text-muted fs-6">
                                                            <i class="fas fa-link me-2"
                                                                aria-hidden="true"></i>{{ $category->category_slug }}
                                                        </small>
                                                    </div>
                                                </td>
                                                <td>
                                                ...
                                                <td class="pe-4 text-end">
                                                    <div class="d-flex justify-content-end gap-2 action-buttons">
                                                        <a href="{{ route('category.edit', $category->id) }}"
                                                            ...
                                                            aria-label="Edit Category {{ $category->category_name }}">
